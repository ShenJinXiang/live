...
## 3. 字符串的解构赋值
字符串也可以解构赋值。这是因为此时，字符串被转换成了一个类似数组的对象，同时类似数组的对象都有一个length属性，因此还可以对这个属性解构赋值
```javascript
...
z: 1
```
上面代码中，函数func的参数表面上是一个数组，但在传入参数的那一刻，数组参数就被解构成变量x、y、z于函数内部的代码来说，它们能感受到的参数就是x、y、z
